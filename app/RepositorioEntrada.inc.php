...
class Repositorioentrada
{
    ...
    public static function obtener_por_fecha($conexion)
    {
        $entradas = [];
        if(isset($conexion))
        {
            try
            {
                ...
                if(count($resultado))
                {
                    ...
                    {
                        $entradas[] = new entrada($fila['id'], $fila['id_autor'], $fila['url'], $fila['titulo'], $fila['texto'], $fila['fecha'], $fila['activa']);
                    }
                }
            } 
            catch (PDOException $ex) 
    ...
    public static function entrada_existe ($conexion, $url)
    {
        ...
        if (isset($conexion))
        {
            try
            {
                ...
                if(!empty($resultado))
                {
                    $entrada = new entrada($resultado['id'], $resultado['id_autor'], $resultado['url'], $resultado['titulo'], $resultado['texto'], $resultado['fecha'], $resultado['activa']);
                }
            } 
            catch (PDOException $ex) 
            {
        ...
        if(isset($conexion))
        {
            try
            {
                ...
                if(count($resultado))
                {
                    ...
                    {
                        $entradas[] = new entrada($resultado['id'], $resultado['id_autor'], $resultado['url'], $resultado['titulo'], 
    ...
    public static function entradas_usr_fecha ($conexion, $id_usuario)
    {
        ...
        if(isset($conexion))
        {
            try
            {
                $sql = "SELECT a.id, a.id_autor, a.url, a.titulo, a.texto, a.fecha, a.activa, COUNT(b.id) AS 'cantidad_comentarios' FROM entradas a LEFT JOIN comentarios b ON a.id = b.id_entrada WHERE a.id_autor = :num GROUP BY a.id ORDER BY a.fecha DESC";
                $sentencia = $conexion -> prepare($sql);
                ...
                if(count($resultado))
                {
                    foreach ($resultado as $fila)
                    {
                        $entradas_obtenidas[] = array(new entrada($fila['id'], $fila['id_autor'], $fila['url'], $fila['titulo'], $fila['texto'], $fila['fecha'], $fila['activa']), $fila['cantidad_comentarios']);
                    }
                }
            } 
            catch (PDOException $ex) 
